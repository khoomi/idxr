...
func (rs *ReviewServiceImpl) CreateListingReview(ctx context.Context, userID, listingID primitive.ObjectID, req models.ReviewRequest) error {
	...
	_, err = dbSession.WithTransaction(ctx, callback, txnOptions)
	if err != nil {
		return err
	}

	return dbSession.CommitTransaction(ctx)
}
...
func (rs *ReviewServiceImpl) DeleteOtherListingReview(ctx context.Context, ownerID, listingID, userToRemoveID primitive.ObjectID) error {
	err := common.VerifyListingOwnership(context.TODO(), ownerID, listingID)
	if err != nil {
		return err
	}
